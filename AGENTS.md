...
| **Agent Server**        | `src/server.ts`         | MCP Server exposing 10 tools (`connect_to_mcp`, `call_mcp_primitive`, `list_mcp_primitives`, `find_tools`, `disconnect_from_mcp`, `mcp_server_status`, `get_mcp_server_stderr`, `list_available_mcp_servers`, `validate_mcp_server`, `search_all_local_mcp_servers`) for dynamic MCP server testing. Uses `registerTool()` with Zod schemas. |
| **Ranking**             | `src/ranking.ts`        | Lexical relevance ranking (`rankTools`) powering `find_tools` — the "Level 2" of Dynamic Context Loading. Returns compact, ranked tool summaries so agents discover the right tool without eager-loading the whole catalog (the "tools tax"). |
...
| **Compression**         | `src/compression.ts`    | Pure helpers for the compressing proxy: format a tool as `<tool>name(args): summary</tool>` per compression level (low/medium/high/max), build the `get_tool_schema` catalog + schema response, flatten MCP results to text, coerce JSON-string args, tool filters. |
| **Proxy**               | `src/proxy.ts`          | `run-mcp proxy` — the transparent compressing proxy (Stage B1): an `McpServer` exposing `get_tool_schema` + `invoke_tool` (+ `list_tools` at max) for a single auto-connected backend, slashing tool-metadata tokens. Routes `invoke_tool` through the interceptor. |
